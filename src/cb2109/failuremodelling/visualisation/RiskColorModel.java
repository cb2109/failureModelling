...
public class RiskColorModel {
    ...
    private final Color[] equationColors = new Color[numberOfIntensitySteps];
    ...
    public RiskColorModel() {
        equationColors[0] = Color.WHITE;
        assetColors[0] = Color.WHITE;
        for (int i = 1; i < numberOfIntensitySteps; i++) {
            equationColors[i] = new Color(255, (numberOfIntensitySteps - i) * intensityColorSteps, 0, intensityAlpha);
    ...
    public Color getColor(double intensity) {
        ...
        return cm == ColorMode.ASSET ? assetColors[intensityLevel] : equationColors[intensityLevel];
    }
